/**
 * KarrotDEX App - With Live Price Integration
 * Fixed version that restores all features
 */
...
const TOKENS = [
  ...
  { symbol: 'KARROT', name: 'Karrot', chainId: 369, logo: 'https://cryptologos.cc/logos/ethereum-eth-logo.png' },
...
const COINGECKO_IDS = {
  ...
  'RH': 'pulsechain', 'HEX': 'hex', 'BNB': 'binancecoin', 'SOL': 'solana',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initial price fetch after delay
  setTimeout(fetchLivePrice, 1000);
});
...
function updateTokenDisplay(type, symbol) {
  ...
  if (type === 'from') {
    ...
    iconDiv.innerHTML = `<img src="${token.logo}" alt="${token.symbol}" onerror="this.parentElement.innerHTML='🟣'">`;
  }
}
...
function getMockRate(from, to) {
  ...
  // Default: try via ETH
  const viaEth = (rates[`${from}-ETH`] || 0.0003) / (rates[`${to}-ETH`] || 0.0003);
  return viaEth || 1;
}
...
async function executeSwap() {
  ...
  // This is a demo - actual swap would call DEX router
  setTimeout(() => {
    showMessage(`Swap simulated: ${amount} ${fromToken} → ${document.getElementById('amountToDisplay').textContent} ${toToken}`, 'success');
  }, 2000);
}
...
console.log('KarrotDEX App loaded — Live prices via CoinGecko API');
